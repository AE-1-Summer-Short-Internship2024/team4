export const calculateNeeds = (householdData) => {
  ...
  const products = {
    food: [
      { id: 1, name: '水', quantity: `${Math.round(initialQuantities.waterQuantity)}L`, url: `${baseUrl}水`, purchased: false, expirationDate:"2025-08-08"},
      { id: 2, name: 'レトルトご飯', quantity: `${Math.round(initialQuantities.riceQuantity)}食`, url: `${baseUrl}レトルトご飯`, purchased: false, expirationDate: "2025-08-08"},
      { id: 3, name: 'レトルト食品', quantity: `${Math.round(initialQuantities.retortFoodQuantity)}個`, url: `${baseUrl}レトルト食品`, purchased: false, expirationDate: "2025-08-08"},
      { id: 4, name: '缶詰(さばの味噌煮、野菜など)', quantity: `${Math.round(initialQuantities.cannedFoodQuantity)}缶`, url: `${baseUrl}缶詰(さばの味噌煮、野菜など)`, purchased: false, expirationDate: "2026-08-08"},
      { id: 5, name: '栄養補助食品', quantity: `${Math.round(initialQuantities.supplementQuantity)}箱`, url: `${baseUrl}栄養補助食品`, purchased: false, expirationDate: "2025-08-08"},
      { id: 6, name: '野菜ジュース', quantity: `${Math.round(initialQuantities.vegetableJuiceQuantity)}本`, url: `${baseUrl}野菜ジュース`, purchased: false, expirationDate: "2025-02-08"},
      { id: 7, name: 'チーズ・プロテインバー等', quantity: `${Math.round(initialQuantities.cheeseProteinBarQuantity)}パック`, url: `${baseUrl}チーズ・プロテインバー等`, purchased: false, expirationDate: "2025-02-08"},
      { id: 8, name: '健康飲料粉末', quantity: `${Math.round(initialQuantities.healthDrinkPowderQuantity)}袋`, url: `${baseUrl}健康飲料粉末`, purchased: false, expirationDate: "2025-08-08"},
      { id: 9, name: '調味料セット', quantity: `${Math.round(initialQuantities.seasoningSetQuantity)}セット`, url: `${baseUrl}調味料セット`, purchased: false, expirationDate: "2026-08-08"},
      { id: 10, name: '乾麺 即席麺', quantity: `${Math.round(initialQuantities.dryNoodlesQuantity)}パック`, url: `${baseUrl}乾麺 即席麺`, purchased: false, expirationDate: "2025-08-08"},
      { id: 11, name: '無洗米', quantity: `${Math.round(initialQuantities.polishedRiceQuantity)}kg`, url: `${baseUrl}無洗米`, purchased: false, expirationDate: "2025-08-08"},
      { id: 12, name: '飲み物', quantity: `${Math.round(initialQuantities.drinkQuantity)}本`, url: `${baseUrl}飲み物`, purchased: false, expirationDate: "2025-02-08"},
      { id: 13, name: 'お菓子', quantity: `${Math.round(initialQuantities.snackQuantity)}パック`, url: `${baseUrl}お菓子`, purchased: false, expirationDate: "2025-02-08"},
      { id: 14, name: '果物の缶詰', quantity: `${Math.round(initialQuantities.cannedFruitQuantity)}缶`, url: `${baseUrl}果物の缶詰`, purchased: false, expirationDate: "2026-08-08"},
      { id: 15, name: '乾物', quantity: `${Math.round(initialQuantities.driedFoodQuantity)}kg`, url: `${baseUrl}乾物`, purchased: false, expirationDate: "2025-08-08"},
      { id: 16, name: 'フリーズドライ食品', quantity: `${Math.round(initialQuantities.freezeDriedFoodQuantity)}kg`, url: `${baseUrl}フリーズドライ食品`, purchased: false, expirationDate: "2025-08-08"},
    ],
    hygiene: [
